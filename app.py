...
def build_index():
    ...
    try:
        ...
        try:
            ...
            print("✅ Loaded UnstructuredWordDocumentLoader")
        except Exception as e:
            raise Exception(f"Failed to import UnstructuredWordDocumentLoader: {e}")

        try:
            from langchain_text_splitters import RecursiveCharacterTextSplitter
            print("✅ Loaded RecursiveCharacterTextSplitter")
        except Exception as e:
            raise Exception(f"Failed to import RecursiveCharacterTextSplitter: {e}")

        try:
            from langchain_community.retrievers import BM25Retriever
            print("✅ Loaded BM25Retriever")
        except Exception as e:
            raise Exception(f"Failed to import BM25Retriever: {e}")

        try:
            from langchain_huggingface import HuggingFaceEmbeddings
            print("✅ Loaded HuggingFaceEmbeddings")
        except Exception as e:
            raise Exception(f"Failed to import HuggingFaceEmbeddings: {e}")

        try:
            from langchain_chroma import Chroma
            print("✅ Loaded Chroma")
        except Exception as e:
            raise Exception(f"Failed to import Chroma: {e}")

        print("⏳ Loading document...")
        print(f"   Looking for file at: {DOCX_PATH}")
        ...
        index_ready = True
        print("✅ Index fully ready! Chatbot is live.")

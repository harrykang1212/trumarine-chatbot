...
# ── Lightweight TF-IDF search (no torch, no sentence-transformers) ──
def tokenize(text):
...
def tfidf_search(query, chunks, tfs, df, k=8):
    """Search chunks using TF-IDF scoring."""
    n = len(chunks)
    query_tokens = tokenize(query)
    scores = []
    for i, (chunk, tf) in enumerate(zip(chunks, tfs)):
        score = 0
    ...
    return [chunks[i] for score, i in scores[:k] if score > 0]
...
def run_chatbot(query: str) -> str:
    ...
    results = tfidf_search(query, chunks_store, tfs_store, df_store, k=8)
